...
model = dict(type='VideoMaskFeat',
             backbone=dict(type='MaskFeatMViT',
                           arch='maskfeat-small',
                           drop_path_rate=0.0,
                           dim_mul_in_attention=False),
             neck=dict(type='LinearNeck',
                       in_channels=768,
                       out_channels=108,
                       with_avg_pool=False,
             ...
             head=dict(type='MaskFeatPretrainHead',
                       loss=dict(type='PixelReconstructionLoss',
                                 criterion='L2')),
             target_generator=dict(type='HOGGenerator3d',
                                   nbins=9,
                                   pool=8,
                                   gaussian_window=16))
...
data_preprocessor = dict(type='VideoDataPreprocessor',
                         mean=[114.75, 114.75, 114.75],
                         std=[57.375, 57.375, 57.375],
                         format_shape='NCTHW')
train_pipeline = [
    dict(type='mmaction.DecordInit'),
    dict(type='mmaction.SampleFrames',
         clip_len=16,
         frame_interval=4,
         num_clips=1),
    ...
    dict(type='MaskFeatMaskGenerator3D',
         input_size=(8, 7, 7),
