...
def test_asymmetric_loss():
    ...
    loss_cfg = dict(type='AsymmetricLoss',
                    gamma_pos=1.0,
                    gamma_neg=4.0,
                    clip=0.05,
                    reduction='mean',
    ...
    assert torch.allclose(loss(cls_score, label, weight=weight),
                          torch.tensor(3.80845 / 6))
...
def test_label_smooth_loss():
    ...
    with pytest.raises(AssertionError):
        loss_cfg = dict(type='LabelSmoothLoss',
...
def test_reconstruction_loss():
    ...
    # test L1 loss
    loss_config = dict(type='PixelReconstructionLoss',
                       criterion='L1',
                       channel=3)
    loss = build_loss(loss_config)
